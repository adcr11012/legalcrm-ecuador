...
export const MANUAL: SeccionManual[] = [
  ...
  },
  {
    ...
    contenido: [
      ...
      '— Feriados nacionales y locales: se mantienen en una tabla que un Administrador del sistema (superadmin) actualiza, porque los feriados se trasladan por decreto (Ley de recuperación de feriados) y no son fechas fijas indefinidas — también sirve para agregar un feriado extraordinario declarado de un momento a otro. Un feriado puede ser nacional (afecta a todo el país) o local de una sola provincia (ej. cantonización, fundación de ciudad) — la calculadora tiene un selector de "Provincia del juzgado" para aplicar solo los feriados locales que correspondan. Si un feriado aparece marcado como "no verificado", conviene confirmarlo antes de confiar en el resultado para un trámite real.',
