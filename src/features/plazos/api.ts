...
export async function upsertFeriado(fecha: string, nombre: string, verificado: boolean): Promise<void> {
  const { error } = await rpc('admin_upsert_feriado', { p_fecha: fecha, p_nombre: nombre, p_verificado: verificado })
  if (error) throw error
}
